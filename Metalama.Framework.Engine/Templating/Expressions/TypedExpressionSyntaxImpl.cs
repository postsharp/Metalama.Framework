        ...
        internal static TypedExpressionSyntaxImpl FromValue( object? value, SyntaxSerializationContext serializationContext )
        {
            switch ( value )
            {
                ...
                default:
                    ...
                    {
                        throw new InvalidOperationException( $"Cannot convert an instance of type {value.GetType().Name} to a run-time expression. If you are attempting to use a run-time expression as dynamic in compile-time code, that is not supported." );
                    }
            }
        }
