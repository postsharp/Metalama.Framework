...
public readonly struct GlobalServiceProvider
{
    ...
    public T GetRequiredService<T>() where T : class, IService =>this.Underlying.GetService<T>() ??  throw new InvalidOperationException( $"Cannot get the service {typeof(T).Name}." );
    ...
    public static implicit operator GlobalServiceProvider( ServiceProvider<IService> serviceProvider ) => new GlobalServiceProvider( serviceProvider );
