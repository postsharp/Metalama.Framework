...
internal class ValidationAspect : FieldOrPropertyAspect
{
    ...
    {
        builder.Advice.AddContract(builder.Target, nameof(ValidatePropertyGetter), ContractDirection.Output,
            args: new { propertyName = builder.Target.Name });

        builder.Advice.AddContract(builder.Target, nameof(ValidatePropertySetter), ContractDirection.Input,
            args: new { propertyName = builder.Target.Name });
    }
...
public sealed class OnPropertyChangedAspect: TypeAspect
{
    ...
    {
        if (meta.Target.FieldOrProperty.Value == value) return;
