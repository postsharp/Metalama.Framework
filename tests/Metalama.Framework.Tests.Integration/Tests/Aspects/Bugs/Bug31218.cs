...
namespace Metalama.Framework.Tests.Integration.Tests.Aspects.Bugs.Bug31218
{
    public sealed class NotNullCheckAttribute : TypeAspect
    {
        public override void BuildAspect(IAspectBuilder<INamedType> builder)
        {
            ...
            foreach (var method in builder.Target.Methods.Where(m =>
                                                                 ...
                                                                 || m.IsExplicitInterfaceImplementation))
            {
                ...
                {
                    builder.Advice.AddContract(parameter, nameof(ValidateParameter),
                ...
                {
                    builder.Advice.AddContract(method.ReturnParameter, nameof(ValidateMethodResult),
                        args: new { methodName = method.Name });
                }
            }

            foreach (var constructor in builder.Target.Constructors.Where(c =>
            ...
            {
                builder.Advice.AddContract(property, nameof(ValidatePropertyGetter), ContractDirection.Output,
                    args: new { propertyName = property.Name });

                builder.Advice.AddContract(property, nameof(ValidatePropertySetter), ContractDirection.Input,
                    args: new { propertyName = property.Name });
            }
        }
    ...
    // <target>
    [NotNullCheck]
    public record SomeRecord(int X, int Y);
}
