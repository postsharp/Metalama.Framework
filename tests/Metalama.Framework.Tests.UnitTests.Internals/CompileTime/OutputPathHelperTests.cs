...
{
    private readonly TestTempFileManager _tempFileManager = new TestTempFileManager();
    ...
    class TestTempFileManager : ITempFileManager
    {
        // We hardcode a long directory so that it does not change according to test environments.
        public string GetTempDirectory( string subdirectory, CleanUpStrategy cleanUpStrategy, Guid? guid = null ) => Path.Combine( @"C:\Users\GaelFraiteur.AzureAD\AppData\Local\Temp\Metalama", subdirectory, "0.5.53.1189-local-GaelFraiteur-debug" );
    }
}