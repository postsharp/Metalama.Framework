...
namespace Caravela.Framework.Impl.UnitTests.Templating.Serialization.Reflection
{
    public class CaravelaGenericsTests : ReflectionTestBase
    {
        ...
        public void FieldInNestedType()
        {
            ...
            var serialized = this._objectSerializers.SerializeToRoslynCreationExpression( CaravelaLocationInfo.Create( CreateCompilation( code ).DeclaredTypes.GetValue().Single().NestedTypes.GetValue().Single().Properties.GetValue().Single() ) )
                .ToString();
