    ...
    public class CaravelaGenericsTests : ReflectionTestBase
    {
        ...
        public void FieldInNestedType()
        {
            ...
            var serialized = this.Serialize(
                    CompileTimeFieldOrPropertyInfo.Create( CreateCompilationModel( code ).Types.Single().NestedTypes.Single().Fields.Single() ) )
                .ToString();
        ...
        public void TestDerivedGenericType()
        {
            ...
            var serialized = this
                .Serialize( CompileTimeType.Create( CreateCompilationModel( code ).Types.Single( t => t.Name == "User" ).BaseType! ) )
                .ToString();

            TestExpression<Type>( code, serialized, info => Assert.Equal( "Target`1[T2]", info.ToString() ) );

            var serialized2 = this
                .Serialize( CompileTimeType.Create( CreateCompilationModel( code ).Types.Single( t => t.Name == "Target" ) ) )
                .ToString();
        ...
        public void TestHalfTypes()
        {
            ...
            var serialized = this.Serialize(
                    CompileTimeMethodInfo.Create( CreateCompilationModel( code ).Types.Single( t => t.Name == "User" ).BaseType!.Methods.First() ) )
                .ToString();
            ...
            var serialized2 = this.Serialize(
                    CompileTimeMethodInfo.Create( CreateCompilationModel( code2 ).Types.Single( t => t.Name == "User" ).BaseType!.Methods.First() ) )
                .ToString();
