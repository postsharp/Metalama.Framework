...
public static class AspectReceiverExtensions
{
    ...
    /// </summary>
    public static IAspectReceiver<INamedType> SelectReflectionType( this IAspectReceiver<ICompilation> receiver, IEnumerable<Type> types )
        => receiver.SelectMany( c => types.Select( t => (INamedType) ((ICompilationInternal) c).Factory.GetTypeByReflectionType( t ) ) );

    /// <summary>
    /// Selects several <see cref="INamedType"/> in the current compilation or in a reference assembly given their reflection <see cref="Type"/>.
    /// </summary>
    public static IAspectReceiver<INamedType> SelectReflectionType( this IAspectReceiver<ICompilation> receiver, Type[] types )
        => receiver.SelectReflectionType( (IEnumerable<Type>) types );
}